...
{
    public class Seq_Drop : _SEQBASE
    {
        public Seq_Drop(MainCtrl main)
        {
            _ctrl = main;
            arg = new TASKARG(eSEQLIST.Drop);
        }
            ...
            try
            {
                switch (arg.nStep)
                {
                    ...
                    case 105:
                        ...
                        break;
                    case 110:
                        if ((!_ctrl.IO_IN(eINPUT.MFC_End_TrayDtc) && !_ctrl.IO_IN(eINPUT.MFC_Cen_TrayDtc)) || _ctrl.IO_IN(eINPUT.MFC_End_TrayDtc))
                        {
                            if (arg.tDly.IsOver())
                            {
                    ...
                    case 120:
                        ...
                        else
                        {
                            ...
                            if (arg.tSen.IsOver())
                            {
                                SetErr(eERROR.TrayDetected, 100);
                            }
                        }
                        break;
                    case 130:
        ...
        private void ConvRun(bool bTrg)
        {
            _ctrl.CONV_SetSpd(_Data.Inst.sys.cfg.pio.nConvSpd_Normal);
            _ctrl.CONV_Motor(bTrg, false);
        }
    }
}
